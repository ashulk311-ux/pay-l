...
export default function ESIConfiguration({ company }) {
  ...
  const { data: configsData, isLoading } = useQuery(
    ['statutoryConfigs', company?.id, 'ESI'],
    () => statutoryService.getConfigurations(company?.id, { statutoryType: 'ESI' }),
  ...
  const createMutation = useMutation(
    (data) => statutoryService.createConfiguration({ ...data, companyId: company.id, statutoryType: 'ESI' }),
    {
      onSuccess: () => {
        toast.success('ESI configuration created successfully');
        queryClient.invalidateQueries(['statutoryConfigs', company?.id]);
        setDialogOpen(false);
        reset();
      },
      onError: (error) => {
        toast.error(error.response?.data?.message || 'Failed to create ESI configuration');
      }
    }
  );
  ...
  const updateMutation = useMutation(
    ({ id, data }) => statutoryService.updateConfiguration(id, data),
    {
      ...
      onError: (error) => {
        toast.error(error.response?.data?.message || 'Failed to update ESI configuration');
      }
    }
  );
  ...
  const deleteMutation = useMutation(
    ...
    {
      onSuccess: () => {
        toast.success('ESI configuration deleted successfully');
        queryClient.invalidateQueries(['statutoryConfigs', company?.id]);
      },
      onError: (error) => {
        toast.error(error.response?.data?.message || 'Failed to delete ESI configuration');
      }
    }
  );
  ...
  const handleDelete = (config) => {
    if (window.confirm(`Are you sure you want to delete ESI configuration for ${config.state}?`)) {
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h6">Employee State Insurance (ESI) Configuration</Typography>
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => handleOpenDialog()}>
          Add ESI Configuration
        </Button>
      </Box>
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="md" fullWidth>
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogTitle>{isEdit ? 'Edit ESI Configuration' : 'Add ESI Configuration'}</DialogTitle>
          <DialogContent>
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
              ...
              <Box sx={{ display: 'flex', gap: 2 }}>
                <Controller
                  ...
                  control={control}
                  render={({ field }) => (
                    <FormControlLabel control={<Switch {...field} checked={field.value} />} label="Enable ESI" />
                  )}
                />
                <Controller
                  name="autoDeduction"
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} md={6}>
                  <Controller
                    name="configuration.esiNumber"
                    control={control}
                    render={({ field }) => (
                      <TextField
                        label="ESI Number"
                        fullWidth
                        value={field.value?.esiNumber || ''}
                        onChange={(e) => field.onChange({ ...field.value, esiNumber: e.target.value })}
                      />
                    )}
                  />
                </Grid>
